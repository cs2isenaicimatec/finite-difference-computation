...
#define SREGSTATE_ON     TRUE
#define SREGSTATE_OFF    FALSE
        ...
        for (sregId=0; sregId<NSHIFTREGS; sregId++){
            ...
            /* mem access pointers update */
            {
                dirPtr[sregId].pcnt  = dirXZ[sregId].xin*NZE + 
                                        dirXZ[sregId].zin; 
                dirPtr[sregId].dcnt  = dirXZ[sregId].xout*NT + ((NT-1) - 
                                        dirPtr[sregId].itcnt);
                ...
                revPtr[sregId].pcnt  = revXZ[sregId].xin*NZE + 
                                        revXZ[sregId].zin;
                revPtr[sregId].dcnt  = revXZ[sregId].xout*NT + ((NT-1) - 
                                        revPtr[sregId].itcnt);
                revPtr[sregId].oidx  = revXZ[sregId].xout*NZE + 
                                        revXZ[sregId].zout;
                revPtr[sregId].imcnt = (revXZ[sregId].xout-NXB)*NZE + 
            ...
            /* input data */
            {
                ...
                if(dirFlags[sregId].sregStateInput==SREGSTATE_ON){
                    if(sregId==0){
                        if(dirPtr[sregId].itcnt==0){// first iteration
                            dirValues[sregId].pVal = snaps1[dirPtr[sregId].pcnt];
                        ...
                        if(dirPtr[sregId].itcnt==1){
                            dirValues[sregId].pVal  = snaps0[dirPtr[sregId].pcnt];
                            dirValues[sregId].ppVal = END_SHIFT_VAL;
                        }else{
                            dirValues[sregId].pVal  = dirValues[sregId].outppVal;
                            dirValues[sregId].ppVal = dirValues[sregId].outpVal;
                        }
                    }
                }else{
                ...
                if(revFlags[sregId].sregStateInput==SREGSTATE_ON){
                    if(sregId==0){
                        revValues[sregId].pVal  = p[revPtr[sregId].pcnt];
                        revValues[sregId].ppVal = pp[revPtr[sregId].pcnt];
                    }else{
                        revValues[sregId].pVal  = revValues[sregId].outppVal;
                        revValues[sregId].ppVal = revValues[sregId].outpVal;
                    }
            ...
            /* calc. laplacian */
            {
                ...
                for (io0=0; io0<ORDER_LENGTH;io0++){
                    volatile float tmp_az = (ch_coefs_z[io0]*p_sreg[sregId][XLAP_START*NZE + io0]);
                    volatile float tmp_ax = (ch_coefs_x[io0]*p_sreg[sregId][io0*NZE + ZLAP_START]);
                ...
                for (io1=0; io1<ORDER_LENGTH;io1++){
                    volatile float tmp_az = (ch_coefs_z[io1]*p_sreg[sregId][XLAP_START*NZE + io1]);
                    volatile float tmp_ax = (ch_coefs_x[io1]*p_sreg[sregId][io1*NZE + ZLAP_START]);
            ...
            /* update output */
            {
                if (dirFlags[sregId].sregStateOutput == SREGSTATE_ON){
                    ...
                    if(dirPtr[sregId].itcnt > 1){
                        ...
                        // iterations 0 and 1 have no point update
                        dirValues[sregId].newppVal = p_sreg[sregId][0];
                    }
                    /*****************************************************/
                ...
                if (revFlags[sregId].sregStateOutput == SREGSTATE_ON){
                    ...
                    revValues[sregId].pptpxl = getTaperxl(revXZ[sregId].xout, revXZ[sregId].zout, ch_taperx);
                    revValues[sregId].pptpxr = getTaperxr(revXZ[sregId].xout, revXZ[sregId].zout, ch_taperx);
                    revValues[sregId].pptpz  = getTaperz(revXZ[sregId].xout, revXZ[sregId].zout, ch_taperz);
                    /*****************************************************/
                    //tapper Z
                    revValues[sregId].tapered_pp  = taper_apply(ppr_sreg[sregId][0], 
                    ...
                    /*****************************************************/
                    // update pp
                    revValues[sregId].newppVal   = update_pp(revValues[sregId].tapered_pp, 
                                            pr_sreg[sregId][0], 
                                            v2dt2_sreg[sregId][0], 
                                            lapr_sreg[sregId][0]);
                    /*****************************************************/
                    /* apply dobs */
                    if (dirXZ[sregId].xout>=NXB 
                        && dirXZ[sregId].xout < NXE-NZB
                        && dirXZ[sregId].zout== dobsz){
                        dirValues[sregId].newppVal += dobs[dirPtr[sregId].dcnt];
                    }
                ...
                if (dirFlags[sregId].savePPFlag==TRUE){
                    ...
                    if((sregId==(NSHIFTREGS-1))&&(dirPtr[sregId].itcnt < NT)){
                       ...
                       pp[dirPtr[sregId].oidx] = p_sreg[sregId][0];
                        ...
                        dirValues[sregId].outpVal  = p_sreg[sregId][0]; // saves p for next it
