...
__kernel void laplacian (   __global const float  * restrict p,   
    ...
    /////////////////////////////////////////////////////////////
    // this is necessary in order to the compiler
    // implement acmz and acmx as shiftregs, not
    // as local memory bank
    /////////////////////////////////////////////////////////////
    int float_conv_v;
        ...
        for (i1=0; i1<NSHIFTREGS; i1++){
            ...
            if (i0 >= PREFETCH_LENGTH-1){
                    ...
                    && z[i1] >= NZ_START && z[i1] < NZ_END)
                {
                    ...
                    laplaces_out[i1][(i0-(PREFETCH_LENGTH-1) + HALF_ORDER)] = 0.f;
                }
                if(z[i1]>=NZ-1){
                    z[i1] = 0;
