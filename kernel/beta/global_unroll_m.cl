    ...
    for (i0=SHIFT_START; i0<(SHIFT_END+(NZ-ORDER_LENGTH)); i0++){
        ...
        for (i1=0; i1<NSHIFTREGS; i1++){
            ...
            for (j1=0; j1<ORDER_LENGTH; j1++){
                acmz[i1][j1] = shiftreg_in[i1][NX_START*NZ + j1] * coefs[j1];
            }
            ...
            if (i0 >= PREFETCH_LENGTH-1){
                if (x[i1] >= xstart[i1] && x[i1] <= (xend[i1]-HALF_ORDER) 
                    ...
                    shiftreg_out_addr[i1] = x[i1]*NZ+z[i1];
                    shiftreg_out_data[i1] = accout[i1];
                }else{
                    shiftreg_out_addr[i1] = 0;
                    shiftreg_out_data[i1] = 0.f;
                }
                if(z[i1]>=NZ-1){
                    z[i1] = 0;
