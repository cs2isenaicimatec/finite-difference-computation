...
# create database. We assume that rtmprof database
# has already been created using PGAdmin
db.open()
query = QSqlQuery(db)
try:
	do_query(query, 'DROP DATABASE ' + dbname)
...
do_query(query, 'CREATE TABLE runtime ('
		'id		bigint		NOT NULL,'
		'time	bigint)')

do_query(query, 'INSERT INTO runtime ('
		'id, "time") VALUES (1, '+ runtime + ')')
...
def copy_output_file(file, table_name):
	...
	conn = PQconnectdb("dbname='" + dbname +"' user='fpga' host='localhost' password='123'")
	if (PQstatus(conn)):
